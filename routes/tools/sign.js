...
const Wallet = new ethers.Wallet("0x" + config.eth_privateKey)
 ...
 * @returns true / false
 */
function signUserMail(publicKey, verifyMessage, signature) {
    verifyMessage = utils.toUtf8Bytes(verifyMessage)
    let signingAddress = utils.verifyMessage(verifyMessage, signature);
    console.log(signingAddress, typeof publicKey);

    return utils.hexlify(signingAddress).toLowerCase() === publicKey.toLowerCase()
}
